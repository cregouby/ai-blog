...
zoo_split <- initial_split(
  zoo_dataset %>% select(-rowname),
  prop = .8, strata = "pathString"
)
# few observation in the test-set to look at
test_ids <- sample.int(nrow(testing(zoo_split)), 8) 
```
...
zoo_hat_hier <- bind_cols(
    predict(zoo_model_hier, zoo_test_tree),
    tabnet:::node_to_df(zoo_test_tree)$y
  )
knitr::kable(
  zoo_hat_hier[test_ids, ]
)
```
...
zoo_hat_multi <- bind_cols(
    predict(zoo_model_multi, zoo_test_multi),
    zoo_test_multi %>% select(starts_with("level_"))
  )
knitr::kable(
  zoo_hat_multi[test_ids, ]
)
