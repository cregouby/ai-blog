...
{recipes} workflow is not natively available for `Node` datasets for now. So we should have prepared the dataset before turning it to a `Node` object, via numerical values normalization as recommended by the authors of [TabNet paper]((https://arxiv.org/abs/1908.07442).  
...
config <- tabnet_config(penalty = .05, decision_width = 6, attention_width = 6, num_steps = 3, 
                        ...
                        valid_split = .1, epoch = 100, verbose = TRUE)
```
### Model training
