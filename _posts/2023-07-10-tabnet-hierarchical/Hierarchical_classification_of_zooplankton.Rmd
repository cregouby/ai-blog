...
zooplanktest <- readRDS(here::here("../_packages/tabnet_experiments/zooplankton_train.rds"))
head(zooplankton, 4)
...
{recipes} workflow is not available for this `Node` dataset for now. So we should have prepared the dataset before turning it to a `Node` object, via numerical values normalization as recommended by the authors. 
We can in fact use {recipes} function to do it, but the full preparation shall be done including (`prep()` + `bake()`)
```{r}
...
We are now ready to fit the hierarchical multi-label classification model in just passing the Node object to it:

```{r}
#| label:model training for 50 epoch
zoo_model <- tabnet_fit(zoo_tree, valid_split = .1, epochs = 50, verbose = T)
```
